...
export default async function(req:NextApiRequest, res:NextApiResponse) {
    ...
    const merchantOrder = await getMerchantOrderId({merchantOrderId: String(id)});
    if(!merchantOrder) return res.status(404).json({message: "La orden que estás solicitando no existe"})
    const response = {
    ...
    if(response.status !== "closed") res.status(400).send("El producto aún no ha sido pagado");
    ...
    res.status(200).json({message: "La compra se realizó con éxito", orderData:order.data});
};
